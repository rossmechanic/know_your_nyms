...
def score_words(base_word, input_words, sem_rel, relations_percentages):
    ...
    words_to_total_scores = {
        word: words_to_esp_scores[word]
        + words_to_wn_bonuses[word]
        + words_to_ch_bonuses[word]
    ...
    return [
        (
            word,
            {
                "esp_score": words_to_esp_scores[word],
...
def word_net(base_word, input_words, sem_rel):
    ...
    words_to_word_net = {
        word: True if stemmer.stem(word) in base_word_pairs else False
        for word in input_words
    }
    words_to_wn_bonuses = {
        k: word_net_bonus if v else 0.0 for k, v in list(words_to_word_net.items())
    }
    ...
    for word in input_words:
        try:
            relation = Relation.objects.get(
                type=sem_rel, base_word=base_word, input_word=word
            )
...
def get_relations_percentages(sem_rel, base_word):
    ...
    percentages = [
        (word, round((float(input_word_dict[word]) / times_played), 3))
        for word in input_word_dict
    ]
...
def get_esp_scores(input_words, relations_percentages):
    ...
    stem_dict = {
        stemmer.stem(word): word for word in list(relations_percentages.keys())
    }
    ...
    return {
        word: float(relations_percentages[stem_dict[stemmer.stem(word)]] * 100)
        if stemmer.stem(word) in stem_dict
        else 0
        for word in input_words
    }
...
def get_overall_player_avg():
    lst = list(
        [div(x.total_score, x.rounds_played) for x in list(UserStat.objects.all())]
    )
...
def get_or_create_conf_stat(sem_rel, base_word, word):
    try:
        relation = ConfirmationStat.objects.get(
            sem_rel=sem_rel, base_word=base_word, input_word=word
        )
    except ObjectDoesNotExist:
        relation = ConfirmationStat.objects.create(
            sem_rel=sem_rel, base_word=base_word, input_word=word
        )
        relation.save()
    return relation
...
def confirm_or_reject_relation(sem_rel, base_word, word, decision):
    try:
        relationset = ConfirmationStat.objects.filter(
            sem_rel=sem_rel, base_word=base_word, input_word=word
        )
...
def save_word_result(word, sem_rel, score, i):
    try:
        ...
        word_stat.avg_score = (
            word_stat.avg_score * word_stat.rounds_played + score
        ) / (word_stat.rounds_played + 1)
...
def mark_played(user, index, word, sem_rel):
    try:
        cmp_stat = CompletedStat.objects.get(
            user=user, sem_rel=sem_rel, index=index, base_word=word
        )
    except ObjectDoesNotExist:
        cmp_stat = CompletedStat.objects.create(
            user=user, sem_rel=sem_rel, index=index, base_word=word
        )
        cmp_stat.save()
    user_stat = get_or_create_user_stat(user)
...
def skip_word(request, conc_rating):
    ...
    pass_object = Pass.objects.create(
        user=request.user, type=sem_rel, base_word=base_word
    )
    pass_object.save()

    passes = float(
        Pass.objects.filter(type=sem_rel, base_word=base_word).distinct().count()
    )
    ...
    if passes + plays >= 50:
        if base_word in conc_rating:
            ...
            if (
                plays / (passes + plays) < 0.5 * percent_known
                and plays / (passes + plays) < factor
            ):
                word_stat = get_or_create_word_stat(base_word, sem_rel, index)
                word_stat.retired = True
                word_stat.save()
        else:
            if plays / (passes + plays) < 0.2:
                word_stat = get_or_create_word_stat(base_word, sem_rel, index)
                word_stat.retired = True
                word_stat.save()
...
def rank(user_stat_arr, user_stat, getStat, lo, hi):
    ...
    if getStat(user_stat) == getStat(user_stat_arr[mid]):
        if user_stat_arr[lo].user.username == user_stat.user.username:
            return mid
        return (
            rank(user_stat_arr, user_stat, getStat, mid + 1, hi)
            + rank(user_stat_arr, user_stat, getStat, lo, mid)
            + 1
        )
...
def dynamic_select_word(user, vocab_size, sem_rel, ind):
    ...
    if user.is_authenticated():
        ...
        # Remove completed questions from prospective choices
            ...
            CompletedStat.objects.filter(user=user, sem_rel=sem_rel)
            .filter()
        ...
        unviable = list(
            [
                x
                for x in list(WordStat.objects.filter(sem_rel=sem_rel).distinct("word"))
                if (
                    abs(x.avg_score - goal_question_avg) > 0.3 * question_std_dev
                    and x.rounds_played >= 5
                )
                or x.retired
            ]
        )
